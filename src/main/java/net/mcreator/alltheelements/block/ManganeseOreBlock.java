...
public class ManganeseOreBlock extends AllTheElementsModElements.ModElement {
	...
	public static class CustomBlock extends Block {
		...
		public void addInformation(ItemStack itemstack, IBlockReader world, List<ITextComponent> list, ITooltipFlag flag) {
			...
			list.add(new StringTextComponent("Fusion Point : 1.246\u00BAC"));
		}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("manganese_ore", "manganese_ore", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 10)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(15, 0, 0, 64))));
		}
	}
}
