...
public class PdOreBlock extends AllTheElementsModElements.ModElement {
	...
	public static class CustomBlock extends Block {
		...
		public List<ItemStack> getDrops(BlockState state, LootContext.Builder builder) {
			...
			return Collections.singletonList(new ItemStack(PdOreBlock.block, (int) (1)));
		}
	}
	@Override
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("pd_ore", "pd_ore", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 4)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(1, 1, 1, 35))));
		}
	}
}
