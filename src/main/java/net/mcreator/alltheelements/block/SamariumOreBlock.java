...
public class SamariumOreBlock extends AllTheElementsModElements.ModElement {
	...
	public static class CustomBlock extends Block {
		...
		public List<ItemStack> getDrops(BlockState state, LootContext.Builder builder) {
			...
			return Collections.singletonList(new ItemStack(SamariumOreBlock.block, (int) (1)));
		}
	}
	@Override
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("samarium_ore", "samarium_ore", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 5)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(1, 30, 30, 40))));
		}
	}
}
